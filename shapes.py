...
class Block (AnimatedSprite):
	...
	def __init__(self, relpos, color, links = [ ], ghost = False):
	...
	def set_color (self, color, ghost = False):
		# Determines block graphic. Refer to block.png in the textures folder.
...
class Shape (object):
	...
	def copy (self, ghost = False):
		...
		for i in range(4):
			newshape.blocks[i] = self.blocks[i].copy(ghost = ghost)
		...
		if self.form != 1: # O shapes don't need to be rotated.
			for block in self.blocks:
				...
				if angle == 90:
					...
					raise ValueError('Tetris blocks can only rotate in increments of 90 degrees.')
...
class Grid (PositionedSurface):
	"""
		...
		their blocks are stored here. Fallen block colors are kept.
	...
	def set_cells (self):
		...
		self.cells = [[Block([i, j], 0) if i < 2 or i > 11 or j > 21 else None for i in range(14)] for j in range(24)]
			...
			for i in range(21, -1, -1):
				...
				if full_row:
					...
					for j in range(2, 12):
						...
						if method > 0:
							self.cells[i][j] = None
					# Splice the old row out and create a new blank row on top.
					if method < 1:
						# Delete the old row.
						self.cells.pop(i)
						# Create new blank row at the top.
						self.cells.insert(0, [Block([i, 0], 0) if i < 2 or i > 11 else None for i in range(14)])
